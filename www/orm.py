...
class ModelMetaclass(type):
    def __new__(cls,name,bases,attrs):
        ...
        attrs['__primay_key__'] = primary_key
...
class Model(dict, metaclass = ModelMetaclass):
        ...
        async def find(cls,pk):
            ...
            rs = await select('%s where `%s`=s'%(cls.__select__,cls.__primay_key__), [pk],1)
            if len(rs) == 0:
                return None
            return cls(**rs[0])        
        ...
        async def update(self):
            ...
            args.append(self.getValue(self.__primay_key__))
            rows = await execute(self.__update__, args)
            if rows !=1:
                logging.warn('failed to update by primary_key: affected rows %s'%rows)

        async def remove(self):
            args = list(self.getValue(self.__primay_key__))
