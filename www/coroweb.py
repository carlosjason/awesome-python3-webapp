...
def has_named_kw_args(fn): #有没有关键字参数
    params = inspect.signature(fn).parameters
    for name,param in params.items():    
        if param.kind == inspect.Parameter.KEYWORD_ONLY:
            return True      
  
def has_var_kw_arg(fn): #有没有可变关键字
    params = inspect.signature(fn).parameters
    for name,param in params.items():
...
class RequestHandler(object): #viewer to controller 
...
def add_routes(app,module_name):    
    ...
    if n == (-1):
        ...
        mod = getattr(__import__(module_name[:n]), globals(),locals(), [name], name) # 动态引入
    for attr in dir(mod):  #获得mode的方法
